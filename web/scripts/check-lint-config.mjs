 ...
 *
 * ---------------------------------------------------------------------------
 * Canonical copy: gitstate/web/scripts/check-lint-config.mjs. This is
 * pango's copy — see the CONFIG block below for what differs. pango's own
 * eslint.config.js header notes the 220-error, 120-implicit-any .jsx->.tsx
 * rename that lint reported green under the untyped `recommended` set —
...
const CONFIG = {
  ...
  // resolves. pango's tsconfig.json has `"include": ["src", "tests/e2e"]` —
  // src is the app surface and is covered by eslint.config.js's
  // `src/**/*.{ts,tsx}` type-aware block.
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). pango has a
  // single package.json, at web/ — the Go backend under backend/ has none.
  // Walking from the repo root anyway keeps this gate correct if a second
  // package.json is ever added (e.g. root-level tooling).
  ...
  // (assertion C). Measured real count at adoption time: 62 files (0 err) —
  // 49 src + 6 e2e (tests/e2e/) + configs. Floor set to 45 — tolerates
  // normal churn while still catching a glob that collapses to a handful of
  // files or zero.
  coverageFloor: 45,
}
